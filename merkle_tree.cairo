...
func create_initial_branches_rec{range_check_ptr}(zerohash: Uint256*, acc: DictAccess*, index: felt, max_index: felt) -> (res: DictAccess*):
    ...
    dict_write{dict_ptr=acc}(key=index, new_value=zerohash_felt_addr)  # implicitly updates acc
...
func add_values{branches: DictAccess*, bitwise_ptr: BitwiseBuiltin*, range_check_ptr}(value: Uint256*, index: felt, max_index: felt):
    ...
    add_value{branches=branches}([value], index) # implicitly updates branches
    return add_values{branches=branches}(value=value+Uint256.SIZE, index=index+1, max_index=max_index) # implicitly updates branches
end
...
func _get_root_from_branches_rec{range_check_ptr, bitwise_ptr: BitwiseBuiltin*, branches: DictAccess*}(zerohashes: Uint256*, size: felt, cur_hash: Uint256, height: felt, max_height: felt) -> (res: Uint256):
    # alloc_locals is  needed for automatic implicit reference rebinding
    # see https://www.cairo-lang.org/docs/how_cairo_works/builtins.html#revoked-implicit-arguments
    alloc_locals
    ...
    let (use_branch_check) = bitwise_and(size, 2)
    if use_branch_check == 1:
        let (branch) = _get_hash_from_branch{branches=branches}(height)
...
func branch_by_branch{range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(values: Uint256*, size: felt) -> (res: Uint256):
    ...
    # squashing to vefiry zerohashes initialization is sound
    let (branches_start, branches_end) = default_dict_finalize(branches_start, branches_end, default_val)
    
    add_values{branches=branches_end}(values, index=0, max_index=size)  # implicitly updates branches_end
    # squashing to vefiry adding values is sound
    let (branches_start, branches_end) = dict_squash(branches_start, branches_end)

    let (local merkle_tree_root: Uint256) = get_root_from_branches{branches=branches_end}(  # implicitly updates branches_end
        zerohashes=zerohashes,
        size=size,
        max_height=TREE_HEIGHT
    )
    # squashing to vefiry get_root_from_branches is sound
    let (branches_start, branches_end) = dict_squash(branches_start, branches_end)
